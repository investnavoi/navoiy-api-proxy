...
function looksLikeSavedMessagesDialog(dialog, me){
  if(!dialog) return false;
  const title = String(dialog?.title || dialog?.name || '').toLowerCase().trim();
  if(
  ...
  return idsMatch(getUserId(entity), getUserId(me));
}

async function fetchSavedMessagesViaDialogs(client, me){
  try {
    const dialogs = await client.getDialogs({limit:50});
    const candidates = (dialogs || []).filter((dialog)=>looksLikeSavedMessagesDialog(dialog, me));
...
async function fetchMessagesForUser(client, user, isSelf, me){
  let messages = [];
  if(isSelf){
    try {
...
async function handleReplies(req, res, body){
  ...
  try {
    ...
    for(const target of targets){
      ...
      const messages = await fetchMessagesForUser(client, user, selfTarget, me);
      ...
      if(!latest && selfTarget && recentMessages.length >= 2){
        latest = recentMessages.find((message)=>{
          const text = normalizeMessageText(getMessageText(message));
          return text && text !== sentMessageNormalized;
        }) || null;
      }
      ...
      const text = getMessageText(latest) || '[Media xabar]';
      replies.push({
        id: target?.id || '',
        replied: true,
        found: true,
