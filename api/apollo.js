...
// people_enrichment uchun id ni URL ga qo'shish
if(action==='people_enrichment' && p.id){url=url.replace('{id}',p.id);delete p.id;}
else if(ep[1].includes('{id}') && p.id){url=url.replace('{id}',p.id);delete p.id;}
